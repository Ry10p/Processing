...
public class Rctmx {
  ...
  private class OrthogonalMap extends Xmap{
    public PVector mapToCanvas(float nx, float ny){
      float x = (nx + (float)0.5) * Rctmx.this.tilewidth;
      float y = (ny + (float)0.5) * Rctmx.this.tileheight;
      return new PVector(x , y);
    }
    public PVector canvasToMap(float x, float y){
      float nx = x / Rctmx.this.tilewidth - (float)0.5;
      float ny = y / Rctmx.this.tileheight - (float)0.5;
    ...
    public void drawTileLayer(PGraphics pg, int m){
      ...
      PVector p;
      Tile tileN;
      Layer l = Rctmx.this.layer[m];
      int xstart = parent.max(0, parent.floor(this.canvasToMap(Rctmx.this.camleft, Rctmx.this.camtop).x - Rctmx.this.drawmargin));
      int xstop = parent.min(Rctmx.this.mapwidth, parent.ceil(this.canvasToMap(Rctmx.this.camleft + pg.width, Rctmx.this.camtop + pg.height).x + Rctmx.this.drawmargin));
      int ystart = parent.max(0, parent.floor(this.canvasToMap(Rctmx.this.camleft, Rctmx.this.camtop).y - Rctmx.this.drawmargin));
      int ystop = parent.min(Rctmx.this.mapheight, parent.ceil(this.canvasToMap(Rctmx.this.camleft + pg.width, Rctmx.this.camtop + pg.height).y + Rctmx.this.drawmargin));
      for(int ny = ystart; ny < ystop; ny++) for(int nx = xstart; nx < xstop; nx++){
        n = l.data[nx + ny * Rctmx.this.mapwidth] - 1;
        if(n >= 0){
          tileN = Rctmx.this.tile[n];
          p = Rctmx.this.mapToCam(nx, ny);
          x = p.x - Rctmx.this.tilewidth / 2 + tileN.offsetx + l.offsetx;
          y = p.y - Rctmx.this.tileheight / 2 + tileN.offsety + l.offsety + (Rctmx.this.tileheight - tileN.image.height);;
  ...
  public Rctmx(PApplet p, String filename){
    this.parent = p;
  ...
  private void loadTmxProperties(XML e){
   ...
   /// Broken!!!!!
    //this.backgroundcolor = 0xff0000; if(e.hasAttribute("backgroundcolor")) this.backgroundcolor = this.readColor(e.getString("backgroundcolor"));
  }
...
//Layers methods
  ...
  public void toImage(int n){
    if(n >= 0 && n < this.layer.length){
      ...
      conv = parent.createGraphics(this.camwidth, this.camheight);//Add Wall
      conv.beginDraw();
